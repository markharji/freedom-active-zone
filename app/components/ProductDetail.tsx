...
export default function ProductDetail({ product }) {
  ...
  // Fetch pending transactions for the selected date
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/apparel-transactions?apparelId=${product._id}&date=${dateString}&status=pending`);
  ...
  const handleSuggestSlots = async () => {
    ...
    try {
      const dateString = selectedDate.format("DD-MM-YYYY");
      const res = await fetch(`/api/apparel-slots?apparelId=${product._id}&date=${dateString}&duration=60`);
      const data = await res.json();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!res.ok) throw new Error(result.message || "Failed to book apparel");
                ...
                render={({ field }) => (
                  <FormControl fullWidth error={!!errors.startTime}>
                    ...
                    <Select
                      ...
                      disabled={loadingTransactions || !selectedDate}
                    >
                      {hours.map((hour) => (
                ...
                render={({ field }) => (
                  <FormControl fullWidth error={!!errors.endTime}>
                    ...
                    <Select
                      ...
                      label="End Time"
                      onChange={(e) => field.onChange(e.target.value)}
                      disabled={loadingTransactions || !startTime || !selectedDate}
                    >
                      {hours.map((hour) => (
                        <MenuItem
                          key={hour}
